...
import { selectLoading } from "features/reports/selectors";
...
const Form: React.FC<Props> = ({ report }) => {
  ...
  const materialsFuncs = {
    ...
    onChange: ({ currentTarget: { value } }: any, key: number) => {
      ...
      const availableMaterials = materialsFuncs.getAvailable(materialsItems, 0);

      materialsItems = materialsItems.filter(({ materialId }) => materialId);
      ...
      <div className={styles.inputs}>
        ...
        {workItems.map(
          ({ workTypeId, roadSignId, volume }, key) =>
            worksFuncs.getAvailable(workItems, workTypeId).length > 0 && (
              <fieldset className={styles.inputGroup} key={key}>
                <Select
                  name={`workTypeId-${key}`}
                  className={styles.select}
                  onChange={(e) => worksFuncs.onChangeWorkType(e, key)}
                  value={workTypeId}
                >
                  <option key={0} value={0}>
                    ------
                ...
                <Input
                  ...
                  className={styles.input}
                  inputClassName={styles.input}
                  onChange={(e) => worksFuncs.onChangeVolume(e, key)}
                  value={volume}
                  disabled={!workTypeId}
                />
              </fieldset>
        ...
        {materialsItems.map(
          ({ materialId, quantity }, key) =>
            materialsFuncs.getAvailable(materialsItems, materialId).length >
              0 && (
              <fieldset className={styles.inputGroup} key={key}>
                <Select
                  name={`materialId-${key}`}
                  className={styles.select}
                  onChange={(e) => materialsFuncs.onChange(e, key)}
                  value={materialId}
                >
                  <option key={0} value={0}>
                    ------
                  ...
                  {materialsFuncs
                    .getAvailable(materialsItems, materialId)
                    .map(({ title, id }) => (
                      <option key={id} value={id}>
                        {title}
                      </option>
                ...
                <Input
                  ...
                  className={styles.input}
                  inputClassName={styles.input}
                  onChange={(e) => materialsFuncs.onChangeQuantity(e, key)}
                  value={quantity}
                  disabled={!materialId}
                />
              </fieldset>
